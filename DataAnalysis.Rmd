...
kable(summarize(group_by(MData,Group),Mean.Width=mean(Width.All.cm),SD.Width=sd(Width.All.cm),Mean.Length=mean(Length.All.cm),SD.Length=sd(Length.All.cm),Mean.Volume=mean(Volume),SD.Volume=sd(Volume)), caption="Table 1: Tabe of summary statistics.")
```
...
kable(select(rename(ungroup(MdataSm),Species=Org),Species,Volume),caption="Table of species and their volumes for species with below-average volume.")
```
